"""Analyze titin aggregates using positive and negative threshold controls."""
...
POSITIVE_NAME = "positive_control.tif"
...
def image_set_names(data_dir: Path) -> list[str]:
    """Return complete non-control C1/C2/C3 image sets."""
    if not data_dir.is_dir():
        raise FileNotFoundError(f"Data directory does not exist: {data_dir}")

    excluded_names = {POSITIVE_NAME, NEGATIVE_NAME}
    names = sorted(
        ...
        and path.name.removeprefix("C1-") not in excluded_names
    )
    if not names:
        raise FileNotFoundError(f"No analysis image sets found in {data_dir}")
...
def calibrate_aggregate_threshold(model) -> float:
    """Find an aggregate threshold from the configured control image sets."""
    positive_cell, positive_aggregate, positive_nuclei = load_image_set(
        DATA_DIR, POSITIVE_NAME
    )
    negative_cell, negative_aggregate, negative_nuclei = load_image_set(
        DATA_DIR, NEGATIVE_NAME
    )

    positive_masks, _, _ = model.eval(
        positive_cell, **CELLPOSE_PARAMETERS
    )
    negative_masks, _, _ = model.eval(
        negative_cell, **CELLPOSE_PARAMETERS
    )
    ...
    print(f"Optimal aggregate threshold: {threshold}")
...
def save_inspection_figures(
    ...
    plot_aggregate_channel_color_labelled(
        ...
        cell_masks,
        cell_index_by_label,
        save_path=image_dir / "transfected_cells.png",
    )


def main() -> None:
    """Calibrate the threshold and analyze every non-control image set."""
    ...
    for image_number, name in enumerate(image_sets, start=1):
        ...
        _, trans_labels, _ = extract_masks(
            aggregate_channel=aggregate_channel,
            cell_masks=cell_masks,
            method="otsu",
